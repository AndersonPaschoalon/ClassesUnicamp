...
void iddle_time(int numberLoops)
{
        int i = 0;
        int j = 0;
        char word[IDDLE_WORD]; 

        for(i = 0; i < numberLoops; i++)
        {   
                for(j=0;j<IDDLE_WORD;j++)
                {   
                        word[j] = 0;
                }    
...
int main(void)
{
	...
	//run this program just in one single thread
	if(mytid != 0)
	{
		while(1);
	}

	//rinitialize this program
	nf_pktout_init();
	nf_pktin_init();      
